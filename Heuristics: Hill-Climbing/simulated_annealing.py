...
def evaluation(timetable):
    ...
    for m in rooms:
        ...
        if total_periods > 6: over_periods += 8
...
def simulated_annealing(init_timetable, num_of_iterations, initial_temp):
...
def iterated_local_search(num_of_inits):
    ...
    for init in inits:
        print(f"Try {cnt}")
        solution, score = simulated_annealing(init, 1000, 10)
        scores.append(score)
...
solution, score, candidate_scores = iterated_local_search(10)
print(*candidate_scores)
print("Best Score: " + str(score))

solution.sort(key = lambda x: x[7])
for n in solution:
    print("Block {}: Class {} by {} in Room {}, start from {} to {}".format(
        n[7], n[0], n[2], n[4], n[6], n[6]+n[1]-1
    ))
